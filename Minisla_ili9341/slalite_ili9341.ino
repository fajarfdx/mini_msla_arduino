...
#include <Adafruit_ILI9341_AS.h> //FOR LCD DRIVER ILI9341
#include <SPI.h>
#include <EEPROM.h>
...
SdFat SD;
//LCD TFT DRIVER
Adafruit_ILI9341_AS tft = Adafruit_ILI9341_AS(tftcs, dc, rst); //FOR LCD DRIVER ILI9341
...
void setup() {
  ...
  tft.init(); //FOR LCD DRIVER ILI9341
  tft.setRotation(3);
  tft.setTextSize(2);
  screenW = tft.width();
  screenH = tft.height();
...
void folderNavi(File dir, int isDown) {
  ...
  tft.fillRect(screenWHalf-82, 130, 164, 36, ILI9341_BLACK);
...
void screen11() {
  ...
  tft.drawCentreString(foldersel, (screenWHalf)+1, 134, 2);
  tft.drawRect((screenWHalf)-85, 123, 170, 50, ILI9341_WHITE);
  arrows();
...
void arrows() {
  ...
  tft.fillTriangle(screenW-50, 135, screenW-30, 135, screenW-40, 120, ILI9341_WHITE );  //up arrow
  tft.fillTriangle(screenW-50, 165, screenW-30, 165, screenW-40, 180, ILI9341_WHITE );  //down arrow
}


void rectscreen() {

  tft.drawRect(screenWHalf-40, 110, 80, 40, ILI9341_WHITE);
}
void cleantextscreen() {
  tft.fillRect(screenWHalf-75, 115, 150, 50, ILI9341_BLACK);
}
